...
import { CloudRain, Disc, Lock, Play, Sparkles, Square, Volume2 } from 'lucide-react-native';
...
async function ensureTrackFile(track: Track): Promise<string> {
  ...
  const base64 = createSoundscapeWavBase64(track.kind, {
    durationSeconds: 8,
    sampleRate: 16000,
    seed: track.seed,
  });
...
export const SoundscapesPlayer: React.FC<Props> = ({ onUnlockPro }) => {
  ...
  const handleTrackPress = async (track: Track) => {
    ...
    if (Platform.OS === 'web') {
      Alert.alert('Install Still', 'Offline soundscapes are available in the installed mobile app.');
      return;
    }

    if (activeTrackId === track.id && status.playing) {
      player.pause();
      return;
    }
    ...
    <View style={styles.card}>
      <View style={styles.headerRow}>
        <View style={styles.headerIcon}>
          <Volume2 size={18} color={Colors.emerald} />
        </View>
        <View style={styles.headerCopy}>
          <Text style={styles.eyebrow}>OFFLINE AUDIO</Text>
          <Text style={styles.title}>Soundscapes</Text>
        </View>
        <View style={styles.localBadge}>
          <View style={styles.localDot} />
          <Text style={styles.localText}>ON DEVICE</Text>
        </View>
      </View>
        ...
        {TRACKS.map((track) => {
          ...
          return (
            <TouchableOpacity
              ...
              accessibilityLabel={`${track.title}${track.isPro ? ', Still Pro preview' : isPlaying ? ', pause' : ', play'}`}
            >
              <View style={styles.trackLeft}>
                <View style={[styles.iconWrap, isActive && styles.iconWrapActive]}>
                  <Icon size={18} color={isActive ? Colors.emerald : Colors.textSecondary} />
                </View>
              ...
              {track.isPro ? (
                <View style={styles.proTag}>
                  <Lock size={12} color={Colors.emerald} />
      ...
      {activeTrackId ? (
        <View style={styles.nowPlayingRow}>
          <View style={[styles.nowPlayingDot, status.playing && styles.nowPlayingDotLive]} />
          <Text style={styles.nowPlayingText}>
            {status.playing ? 'Looping locally' : 'Paused'} • {TRACKS.find((track) => track.id === activeTrackId)?.title}
          </Text>
        </View>
      ) : null}
    </View>
